...
`/api/sentence-variants/:id/register`、`/api/audio/:id`、`/api/anki-exports/tsv`はPhase 1で削除した。音声生成はPhase 4でAPKG生成フローへ統合する。音声previewが将来必要になった場合も、米国英語assetを読み取り専用で再生し、生成ボタンは追加しない。

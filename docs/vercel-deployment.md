...
After deploying the current source and applying migrations through `0010`, sign in as the owner and request `GET /api/diagnostics?probe=1`. The probe reports only boolean/status information for the database connection, SayDeck expression schema, and owner AI provider. It verifies the situation master, entry assignments, sequence counter, and new expression columns without returning `DATABASE_URL`, API keys, raw provider responses, or connection strings.

If the probe reports `Database connection: 未接続`, check the Production `DATABASE_URL` value and Neon network access. If it reports `expression schema: 未適用`, confirm that migrations through `0010` were applied in order. If it reports `AI connection: 未接続`, check `OWNER_AI_KEY` and `OWNER_AI_MODEL=grok-4.3` in the Production environment.

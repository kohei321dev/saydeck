...
ユーザーは`言いたいこと`を日本語で入力する。以前使った主シチュエーションを任意で優先できる。AIは登録済み主分類と照合し、主・副シチュエーション、必要な意味単位、必須の`01_標準表現`、差がある場合だけ`02_ネイティブ・口語表現`と`03_表現パターン`を提案する。01は1文・原則18語以内で必要な詳細を含む、その場で使える標準表現とする。複数の発話行為は意味単位へ分割する。ユーザーが分類と候補を確認した時点でDBへ登録する。
...
- `03_表現パターン`は、文法展開、熟語・句動詞、コーパスに基づくコロケーションの完成英文だけを扱う。

...
ユーザーは`言いたいこと`を日本語で入力する。以前使った主シチュエーションを任意で優先できる。AIは登録済み主分類と照合し、主・副シチュエーション、必要な意味単位、必須の`01_基本表現`、差がある場合だけ任意レイヤーを提案する。`01_基本表現`は難易度順の最下層ではなく、1文・原則12語以内で1つの発話行為を標準的に伝える最小表現とする。理由・条件・数量・追加依頼を要する内容は分割するか`02_詳細表現`へ置く。ユーザーが分類と候補を確認した時点でDBへ登録する。
...
- `01_基本表現`へ長い条件文や複数の依頼を詰め込まず、`02_詳細表現`は02a〜02eの適用可能な文法・語句パターンだけを生成する。`03_会話表現`が`01`より短くても矛盾しない。
- `04_ネイティブ表現`は、ネイティブ話者が実際に使う自然な定型句・省略・別構文に限定する。

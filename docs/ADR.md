...
生成AIとの対話中に複数の選択肢が短時間で出る場合でも、最終的に採用した判断だけでなく、捨てた案、前提、再検討条件を短く残す。
...
すべてのADRで全セクションを埋める必要はない。ただし、外部サービス、DB、認証、AI APIでは `Security / Privacy` と `Operations` を必須にする。
...
- 『ソフトウェアアーキテクチャの基礎 第2版』21章: アーキテクチャ決定を記録し、決定理由を伝達する
- 『ソフトウェアアーキテクチャの基礎 第2版』27章: アーキテクチャ判断はトレードオフとして扱う
- 『システム思考の世界へ』1章、8章: 複雑な変更では線形な因果だけでなく、相互依存とフィードバックを扱う

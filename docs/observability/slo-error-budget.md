...
- `/input` のINPUT画面表示
- `/api/expressions/:id/generate` のAI生成
...
|---|---|---:|---|
| INPUT page | `/input` が認証判定後にINPUTまたは明示的なsignin/setup状態を返す | 99% weekly | route, status, code, durationMs |
| AI generation | `/api/expressions/:id/generate` が成功、または分類済みエラーを返す | 95% weekly | route, operation, provider, model, code, durationMs |
| Expression and export persistence | DB利用時のread/writeが成功、または分類済みDBエラーを返す | 99% weekly | route, operation, resource, code, durationMs |
| Authorization | owner-only / unauthenticatedを期待どおり判定する | 99.5% weekly | route, role, code, status |
...
4. 影響範囲がINPUT、LISTS、EXPORTのどこかを分ける
...
- owner以外の権限エラーが意図した403になっているか
- quota / rate limitが `external_ai_quota_exceeded` に分類されているか

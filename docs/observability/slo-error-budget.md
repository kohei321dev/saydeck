...
Scene Builderの初期SLO、失敗分類、error budgetの扱いを定義する。
...
- `/` の学習画面表示
- `/api/review` のAI添削
- `/api/cards` と `/api/practice/*` と `/api/notes` のDB read/write
- owner / viewer の権限判定
...
|---|---|---:|---|
| Learning page | `/` が認証判定後に学習UIまたは明示的なsignin/setup状態を返す | 99% weekly | route, status, code, durationMs |
| AI review | `/api/review` が成功、または分類済みエラーを返す | 95% weekly | route, operation, provider, role, model, code, durationMs |
| Card and practice persistence | DB利用時のread/writeが成功、または分類済みDBエラーを返す | 99% weekly | route, operation, resource, code, durationMs |
| Authorization | owner-only / viewer allowed / unauthenticatedを期待どおり判定する | 99.5% weekly | route, role, code, status |
...
| AI provider unavailable | AI review SLO failure, provider dependency noted | `external_ai_unavailable` |
| AI quota, credit, or rate limit | AI review SLO failure, budget/quota follow-up | `external_ai_quota_exceeded` |
| Vercel platform outage | Track separately from app bug | `server_error` with platform note if known |

外部依存が原因でも、UIが未分類の500や曖昧なmessageだけを返した場合は、Scene Builder側の改善対象にする。

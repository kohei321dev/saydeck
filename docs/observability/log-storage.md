...
[判断] Neon/Postgresにはapplication logを保存しない。学習データ用DBと運用ログを混ぜると、DB負荷、retention、private data削除、検索用途が衝突するため。
...
| Neon table | audit/event data | Do not use for application logs | 学習DBとログDBの責務が混ざるため避ける |
...
- `/api/review` のAI provider failure確認
- DB未設定、migration未適用、query failureの確認
- owner/viewer権限判定の失敗確認

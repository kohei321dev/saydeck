...
[判断] 代表APIは `/api/expressions/:id/generate` とする。表現生成は外部provider、model設定、入力validationが絡むため、request IDの効果を確認しやすい。
...
`/api/expressions/:id/generate` のログfield:
...
3. DB helper、AI provider、TTS adapterの内部ログ

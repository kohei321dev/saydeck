...
[判断] 初期導入は `/api/review` に限定する。AI reviewは外部provider、role、model設定、入力validationが絡むため、代表APIとしてrequest IDの効果を確認しやすい。

## Request ID Rules

- 許可するincoming IDは8〜128文字の英数字、`.`、`_`、`:`、`-` のみ
- 許可外のincoming IDはログに残さず破棄し、server生成IDに置き換える
- request body、学習者の回答文、AI prompt、provider raw responseはrequest contextに含めない
...
1. `/api/cards`、`/api/cards/generate`、`/api/cards/[cardId]`
2. `/api/practice/*` と `/api/notes`
3. DB helperとAI provider adapterの内部ログ

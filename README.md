...
PR Previewで一時URLのGitHub callback mismatchを避けてowner UIを確認する場合は、Vercel Preview environmentにだけ `PREVIEW_AUTH_BYPASS_SECRET` を設定し、Protected Preview上で `/api/preview-auth?token=<secret>` にアクセスします。Productionには設定しません。詳細は `docs/vercel-deployment.md` を参照してください。
